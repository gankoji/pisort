...
int main()
{
	...
	printf("Loop statistics: %ld outer loops, %ld inner loops, %lld bytes memory\n",
	...
	for (long i=0; i< outerLoop; i++)
	{
		...
		recordsInBlock = readMemoryLoadByBlocks(inputFile, arr, M, B, recordSize);
		//readMemoryLoadByBlocks(inputFile, arr, M, B, recordSize);
		...
		writeMemoryLoadByBlocks(outputFile, arr, B, recordSize);
	}
